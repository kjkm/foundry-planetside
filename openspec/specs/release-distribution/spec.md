...
Distribute the module as an installable, auto-updatable GitHub Release produced from a version tag. Defines what triggers a release, what the published artifacts contain (including the build-time-vendored Three.js that is not committed), and how the `manifest`/`download`/`version` fields must be shaped so Foundry can install the module from a stable URL and detect future updates.
