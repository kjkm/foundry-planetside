...
The marker SHALL reflect the ping's style (`options.style`): an `alert` ping SHALL render visually distinct from a normal (`pulse`/default) ping — an urgent appearance (e.g. red, larger/sharper pulse) that overrides the user color — so an alert is immediately distinguishable on the globe. An unrecognized or absent style SHALL fall back to the default user-colored marker.
...
#### Scenario: An alert ping renders distinctly

- **WHEN** an alert ping is drawn (`options.style` is `"alert"`)
- **THEN** the globe marker renders in the distinct alert appearance rather than the normal user-colored pulse
