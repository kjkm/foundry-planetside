...
The atmosphere's **halo color** and overall **intensity** SHALL be configurable per scene (via `flags.planetside.*`), defaulting to the current built-in values. The halo color SHALL tint the outer shell (the visible halo hue), and the intensity SHALL scale both shells' base intensities together. Changes SHALL apply **live, without rebuilding geometry** (updating the shells' shader uniforms). Both shells SHALL track the sun direction configured for the scene.
...
#### Scenario: Atmosphere color and intensity are configurable and apply live
